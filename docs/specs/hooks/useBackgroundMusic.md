...
child-facing screen is active *and the app is in the foreground* — the
soundtrack a toddler hears while playing, and what `SettingsScreen`'s
volume slider and mute button act on. Minimising the app (or locking the
screen) silences it.
...
two per-platform bundle paths below. More effects keep the live instance
in sync with props and the app's foreground state:
...
- `enabled` **and the app is in the foreground** → `sound.play()`;
  otherwise `sound.pause()` — but only once `sound.isLoaded()` (see
  below). The foreground half comes from an `AppState` listener: a
  `'change'` to anything other than `'active'` (home button, app switcher,
  screen lock) flips an internal `appActive` flag off and pauses the
  track; coming back to `'active'` resumes it if `enabled`. The listener
  is torn down on unmount (`subscription?.remove?.()`, guarded for
  RN/mock shapes that return nothing). The internal `shouldPlay =
  enabled && appActive` is what the play/pause effect and the load
  callback actually key off.
...
file is still loading, the callback reads the *current* `shouldPlay`/
`volume`/`muted` from a ref rather than the values captured at
construction. The `volume`/`muted` and `shouldPlay` effects still fire
during loading; they're harmless no-ops then (the library guards
`setVolume` internally; the `shouldPlay` effect bails on `!isLoaded()`),
and the load callback applies the right state once ready. An earlier version called all three
...
- App sent to the background while `enabled` (home button, app switcher,
  incoming call, screen lock) → playback pauses; returning to the
  foreground resumes it. `enabled` itself is unchanged throughout.
...
1. `enabled: true` → `play()` is called. Then `enabled: false` → `pause()`
   is called.
2. With `enabled: true` and the track loaded, an `AppState` `'change'` to
   `'background'` → `pause()`; a following `'change'` to `'active'` →
   `play()` again.
4. Mount with `enabled: true` before the async load resolves → `play()` is
   not called synchronously, but is called once loading finishes.
5. After loading, `setNumberOfLoops(-1)` has been called (loop forever);
   it is not called before loading, when the library would ignore it.
6. `muted: true` → `setVolume(0)` regardless of `volume`. `muted: false` →
   `setVolume(volume)`.
7. Unmount → `stop()` and `release()` are both called.
...
instance methods no-op until then) so scenarios 4–5 are meaningful; tests
`await act()` after mounting to flush it. Scenario 2 spies on
`AppState.addEventListener` to capture the `'change'` handler and invokes
it directly.
